...
def launch_prospective_state(revision,content=Path('/content')):
    ...
    previous=content/'menia-results/generation-numerics-v1/execution.json'
    if not python.is_file() or not previous.is_file() or json.loads(previous.read_text())['status']!='completed':
        raise RuntimeError('Completed numerical control and existing environment required')
    ...
    root=content/'menia-results/prospective-state-v1'; root.mkdir(parents=True,exist_ok=True)
    journal=root/'prospective-state-20260920-v1.jsonl'; status=root/'execution.json'
    archive=content/'menia-prevision-etat-v1.zip'; repo=content/('menia-prospective-state-source-'+revision[:12])
    ...
    try:
        ...
        command([python,'-m','unittest','tests_language.test_prospective_cache_interventions',
            'tests_language.test_prospective_state_discovery','tests_language.test_confidence_generation_numerics',
        ...
        command([python,'-m','research.prospective_state_discovery_gpu','--journal',journal],repo)
        ...
        with zipfile.ZipFile(archive,'x',compression=zipfile.ZIP_DEFLATED) as z:
            for file in sorted(root.iterdir()):
                if file.is_file() and file.suffix in ('.json','.jsonl','.log','.safetensors'): z.write(file,'prospective-state/'+file.name)
