"""One fixed Colab18 attempt, preserving prior outputs and requiring their parent."""
import datetime
import json
import os
...
def launch_native_choices(revision, content=Path('/content')):
    ...
    parent=content/'menia-results/natural-error-v1/natural-error-20260919-v1.jsonl'
    if not python.is_file() or not parent.is_file():raise RuntimeError('Conserver le runtime Menia A100 et le journal complet du Colab17.')
    root=content/'menia-results/native-choice-v1'
    root.mkdir(parents=True,exist_ok=True)
    status=root/'execution.json'
    journal=root/'native-choice-20260919-v1.jsonl'
    archive=content/'menia-choix-natifs.zip'
    repo=content/('menia-native-choice-source-'+revision[:12])
    ...
    try:
        ...
        command([python,'-m','unittest','tests_research.test_native_choice','tests_language.test_native_choice_gpu','-v'],repo)
        manifest['status']='running'
        status.write_text(json.dumps(manifest,indent=2)+'\n',encoding='utf-8')
        command([python,'-m','research.native_choice_gpu',journal,'--parent',parent],repo)
        ...
        with zipfile.ZipFile(archive,'x',compression=zipfile.ZIP_DEFLATED) as z:
            for path in sorted(root.iterdir()):
                if path.is_file() and path.suffix in ('.json','.jsonl','.log'):z.write(path,'native-choice/'+path.name)
