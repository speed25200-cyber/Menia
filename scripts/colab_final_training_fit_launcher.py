"""One read-only diagnostic of final Colab24 weights; no automatic rerun."""
import datetime
import json
import os
...
def launch_final_training_fit(revision, content=Path('/content')):
    ...
    parent = content/'menia-results/confidence-ranking-v1'
    if not python.is_file(): raise RuntimeError('Existing Menia Python environment required')
    if json.loads((parent/'execution.json').read_text())['status'] != 'completed': raise RuntimeError('Completed Colab24 required')
    root = content/'menia-results/final-training-fit-v1'; root.mkdir(parents=True,exist_ok=True)
    journal = root/'final-training-fit-20260920-v1.jsonl'; status = root/'execution.json'
    archive = content/'menia-ajustement-final-v1.zip'; repo = content/('menia-final-training-fit-source-'+revision[:12])
    if journal.exists() or status.exists() or archive.exists(): raise RuntimeError('Existing attempt; preserve it, no automatic retry')
    ...
    try:
        ...
        command([python,'-m','unittest','tests_research.test_confidence_final_training_fit','tests_language.test_answer_confidence_gpu','-v'],repo)
        manifest['status'] = 'running'; status.write_text(json.dumps(manifest,indent=2)+'\n')
        command([python,'-m','research.confidence_final_training_fit_gpu',journal,parent],repo)
        ...
        with zipfile.ZipFile(archive,'x',compression=zipfile.ZIP_DEFLATED) as z:
            for file in sorted(root.iterdir()):
                if file.is_file() and file.suffix in ('.json','.jsonl','.log'): z.write(file,'final-training-fit/'+file.name)
