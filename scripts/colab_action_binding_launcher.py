"""One fixed Colab20 attempt; preserve all preceding experiment outputs."""
import datetime
import json
import os
...
def launch_action_binding(revision, content=Path('/content')):
    ...
    root=content/'menia-results/action-binding-v1'
    root.mkdir(parents=True,exist_ok=True)
    status=root/'execution.json'
    journal=root/'action-binding-20260919-v1.jsonl'
    archive=content/'menia-liaison-actions.zip'
    repo=content/('menia-action-binding-source-'+revision[:12])
    ...
    try:
        ...
        command([python,'-m','unittest','tests_research.test_action_binding','tests_language.test_action_binding_gpu','-v'],repo)
        manifest['status']='running'
        status.write_text(json.dumps(manifest,indent=2)+'\n',encoding='utf-8')
        command([python,'-m','research.action_binding_gpu',journal],repo)
        ...
        with zipfile.ZipFile(archive,'x',compression=zipfile.ZIP_DEFLATED) as z:
            for path in sorted(root.iterdir()):
                if path.is_file() and path.suffix in ('.json','.jsonl','.log','.safetensors'):z.write(path,'action-binding/'+path.name)
