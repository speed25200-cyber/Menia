"""One frozen Colab24 attempt; retain failed attempts and previous experiments."""
import datetime
import json
import os
...
TESTS = ('tests_research.test_confidence_ranking',
         'tests_research.test_confidence_ranking_study',
         'tests_language.test_confidence_ranking_gpu',
         'tests_language.test_output_confidence_trace')


def launch_confidence_ranking(revision, content=Path('/content')):
    ...
    root=content/'menia-results/confidence-ranking-v1'
    root.mkdir(parents=True,exist_ok=True)
    status=root/'execution.json';journal=root/'confidence-ranking-20260920-v1.jsonl'
    archive=content/'menia-classement-confiance-v1.zip'
    repo=content/('menia-confidence-ranking-source-'+revision[:12])
    ...
    try:
        ...
        command([python,'-m','research.confidence_ranking_learning_gpu',journal,repo/'artifacts/answer-confidence-training-data'],repo)
        ...
        with zipfile.ZipFile(archive,'x',compression=zipfile.ZIP_DEFLATED) as z:
            for path in sorted(root.iterdir()):
                if path.is_file() and path.suffix in ('.json','.jsonl','.log','.safetensors'):z.write(path,'confidence-ranking/'+path.name)
