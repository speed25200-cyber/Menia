"""Run one fixed exploratory native state-forecast test on a free A100."""
import datetime
import json
import os
...
def launch_retained_reader_control(revision,content=Path('/content')):
    ...
    previous=content/'menia-results/prospective-pair-v1/execution.json'
    if not python.is_file() or not previous.is_file() or json.loads(previous.read_text())['status']!='completed':
        raise RuntimeError('Completed single-pair control and existing environment required')
    ...
    root=content/'menia-results/retained-reader-control-v1'; root.mkdir(parents=True,exist_ok=True)
    journal=root/'retained-reader-control-20260920-v1.jsonl'; status=root/'execution.json'
    archive=content/'menia-lecteur-controle-v1.zip'; repo=content/('menia-retained-reader-control-source-'+revision[:12])
    ...
    try:
        ...
        command([python,'-m','unittest','tests_language.test_retained_state_reader',
            'tests_language.test_prospective_learning_data','tests_language.test_prospective_cache_interventions',
            'tests_language.test_confidence_cached_action_decode','-v'],repo)
        manifest['status']='running'; status.write_text(json.dumps(manifest,indent=2)+'\n')
        command([python,'-m','research.retained_reader_control_gpu','--journal',journal],repo)
        ...
        with zipfile.ZipFile(archive,'x',compression=zipfile.ZIP_DEFLATED) as z:
            for file in sorted(root.iterdir()):
                if file.is_file() and file.suffix in ('.json','.jsonl','.log','.safetensors'): z.write(file,'retained-reader-control/'+file.name)
