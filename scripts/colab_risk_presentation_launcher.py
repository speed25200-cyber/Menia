"""One fixed Colab19 attempt; preserve all preceding experiment outputs."""
import datetime
import json
import os
...
def launch_risk_presentation(revision, content=Path('/content')):
    ...
    root=content/'menia-results/risk-presentation-v1'
    root.mkdir(parents=True,exist_ok=True)
    status=root/'execution.json'
    journal=root/'risk-presentation-20260919-v1.jsonl'
    archive=content/'menia-presentation-risque.zip'
    repo=content/('menia-risk-presentation-source-'+revision[:12])
    ...
    try:
        ...
        command([python,'-m','unittest','tests_research.test_risk_presentation','tests_language.test_risk_presentation_gpu','-v'],repo)
        manifest['status']='running'
        status.write_text(json.dumps(manifest,indent=2)+'\n',encoding='utf-8')
        command([python,'-m','research.risk_presentation_gpu',journal],repo)
        ...
        with zipfile.ZipFile(archive,'x',compression=zipfile.ZIP_DEFLATED) as z:
            for path in sorted(root.iterdir()):
                if path.is_file() and path.suffix in ('.json','.jsonl','.log'):z.write(path,'risk-presentation/'+path.name)
