"""Three fixed replications, all adapters frozen before crossed evaluation."""
import json
import math
import os
...
def initialize(modules, seed):
    torch.manual_seed(seed)
    with torch.no_grad():
        for module in modules.values():
            nn.init.kaiming_uniform_(module.a, a=math.sqrt(5)); module.b.zero_(); module.enabled=True
    return adapter_state(modules)
...
def run(path, *, resume=False):
    ...
    metadata.update(origin='transformers_gpu', models={'A':study.MODEL},
        settings=dict(dtype='bfloat16',sampling=False,thinking=False,useCache=False,firstTokenOnly=True,
                      attention='sdpa',optimizer='AdamW',weightDecay=0.,initializations=list(study.INITIALIZATIONS),**study.CONFIG))
    ...
    blocks = {b['id']:b for b in fixed['blocks']}; cache = {}; current_rep = None; initial = None
    ...
    for unit in fixed['training']:
        ...
        if unit['replication']!=current_rep:
            initial=initialize(modules,unit['initializationSeed']); current_rep=unit['replication']
        ...
        for step,group in enumerate(unit['groups'],1):
            ...
            if step%8==0: print(f'{key} : mise à jour {step}/{len(unit["groups"])}',flush=True)
    ...
    for index,request in enumerate(fixed['evaluation'][len(rows):],len(rows)+1):
        ...
        with torch.inference_mode():
            logits,stats=evaluate_forward(model,inputs,spans,b,request['family'],request['position'])
        ...
        if index%128==0: print(f'Évaluation {index}/{len(fixed["evaluation"])}',flush=True)
