...
def run(path):
    ...
    try:
        ...
        for case in p['cases']:
            ...
            indices=study.permutation(len(actual.prefix)); args=dict(model_state_id=identity,layers=p['intervention']['layers'])
            ...
            writer.write(dict(event='state',case=case,text=text,metrics=metrics,trace=trace,
