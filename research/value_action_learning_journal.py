...
from research.value_action_learning import make_plan,source_hash,training_batch,request_for
...
def read_journal(path):
    ...
    for e in events[1:]:
        ...
        if e['event']=='training_start':
            require(not results and pending is None and active is None and len(checkpoints)<9,'Training after evaluation or overlap')
            ...
            pair=f'r{unit["replication"]}-choice'
            ...
            require(type(e['seconds']) in (int,float) and math.isfinite(e['seconds']) and e['seconds']>=0,'Training duration')
            ...
            require(type(e['trainingSeconds']) in (int,float) and math.isfinite(e['trainingSeconds']) and e['trainingSeconds']>=0,'Adapter duration')
            checkpoints[e['key']]=e;active=None
        elif e['event']=='request':
            require(len(checkpoints)==9 and active is None and pending is None and len(results)<p['planned'],'Unfrozen evaluation')
