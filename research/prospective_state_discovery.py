"""Fixed exploratory test of native forecasts under hidden cache permutations."""
import argparse
import hashlib
import json
import math
...
PREPARATION = Path(__file__).resolve().parents[1]/'artifacts/prospective-state-preparation/design.json'
CONDITIONS = ('actual', 'same_schedule', 'values_permuted', 'joint_permuted', 'restored')
FILES = ('prospective_state_discovery.py','prospective_state_discovery_gpu.py','prospective_cache_interventions.py',
    'confidence_generation_continuity.py','confidence_generation_numerics.py','confidence_cached_action_decode.py',
    'confidence_action_decode.py','confidence_prefix_interventions.py','answer_confidence_data.py',
    'audit_generation_numerics.py','natural_error_journal.py','cross_model_gpu.py','cross_model_prediction.py',
    'iphone_coupling_report.py','iphone_capability_learning_report.py')
...
def plan():
    keys = ['LUMA','NERI','VAKO','SUDI','PELA','TOVI','ZERA','MIKO']; cases=[]
    for count in (4,8):
        for target in range(count):
            for label in (0,1):
                others=sorted((j for j in range(count) if j!=target),
                    key=lambda j:digest(['menia-prospective-discovery-v1',count,target,j]))
                positives=set(others[:count//2-label]); values=[int(j in positives) for j in range(count)]
                values[target]=label
                table='\n'.join(f'{key} = {value}' for key,value in zip(keys,values))
                task=f'Quelle valeur est associée à {keys[target]} dans le tableau mémorisé ? Réponds uniquement par 0 ou 1.'
                forecast=('Avant de répondre à la prochaine question, prévois si ta réponse sera correcte. '
                    'La question sera : '+task+' Ne donne pas sa réponse maintenant. '
                    'Réponds uniquement par 0 si tu prévois une réponse incorrecte, ou par 1 si tu prévois une réponse correcte.')
                cases.append(dict(id=len(cases),bindings=count,targetIndex=target,targetKey=keys[target],expected=label,
                    values=values,seed=2026092030+len(cases),
                    messages=[dict(role='system',content='Suis le format demandé à chaque tour.'),
                        dict(role='user',content='Mémorise ce tableau pour une question ultérieure :\n'+table+'\nRéponds uniquement OK pour confirmer.')],
                    queries=dict(forecast=forecast,task=task)))
    return dict(schema='menia-prospective-state-discovery-plan-v1',model=MODELS['A'],
        settings=dict(SETTINGS,max_new_tokens=32),maxContextTokens=2048,cases=cases,conditions=list(CONDITIONS),
        intervention=dict(layers=list(range(36)),permutation='rotate prefix positions by floor(length/2)',
            valuesMode='values_only',jointMode='keys_and_values',restoration='inverse values permutation'),
        branches=dict(forecast=dict(codes=['0','1'],negative='incorrect',positive='correct'),
            task=dict(codes=['0','1'],negative='zero',positive='one')),
        exportActualCacheCases=[0,8],plannedGenerations=24,plannedForecasts=120,plannedTasks=120,
        forecastDirectionMargin=.01,weightUpdates=0,
        order='All 24 states, then all 120 forecasts, then all 120 tasks; condition order rotates with case id.',
        strictControls=['same_schedule_cache_exact','restored_cache_exact','tokens_unchanged','parameters_unchanged',
            'same_schedule_branches_exact','restored_branches_exact'],
        failurePolicy='Preserve one attempt. Stop on technical exception or strict-control failure; no retry, replacement, threshold adjustment or case exclusion.',
        scope='Exploratory feasibility of reversible task consequences and untrained native forecasts. Not a confirmatory test, trained text-only comparison, utility trial, consciousness criterion or novelty claim.')
...
def permutation(length):
    shift=length//2
    return list(range(shift,length))+list(range(shift))
...
def read_journal(path,tokenizer,*,require_actual=True):
    ...
    for e in events[1:]:
        ...
        if kind=='state':
            ...
            indices=permutation(len(prefix))
            ...
            if exported is not None:
                require(exported['file']==f'prospective-state-20260920-v1.case{case["id"]:02d}.safetensors' and
...
def summarize(data):
    ...
    return dict(schema='menia-prospective-state-discovery-summary-v1',planHash=data['header']['planHash'],sourceHash=data['header']['sourceHash'],
