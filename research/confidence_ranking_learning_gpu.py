"""Nine matched native-confidence adapters, all frozen before new evaluation."""
import argparse
import json
import math
import os
...
from research.confidence_ranking import paired_batches, preparation_report, CONFIG, ARMS
...
def run(path, data_directory):
    ...
    data, parent_report = load_training(data_directory); rows = data['measured']; p = study.make_plan()
    ...
    writer.write(dict(event='header', plan=p, planHash=study.digest(p), sourceHash=study.source_hash(),
        trainingData=rows, trainingReport=preparation_report(rows), parentTrainingReportHash=study.digest(parent_report),
        origin='transformers_gpu', metadata=metadata), create=True)
    try:
        modules = install_adapters(model, rank=CONFIG['rank'])
        parameters = [v for m in modules.values() for v in (m.a, m.b)]
        if any(m.scale != CONFIG['scale'] for m in modules.values()): raise ValueError('Adapter scale')
        if {id(v) for v in parameters} != {id(v) for v in model.parameters() if v.requires_grad}: raise ValueError('Trainable weights')
        checkpoints = {}
        for rep in range(3):
            ...
            initial = adapter_state(modules); initial_file = path.with_suffix(f'.r{rep}-initial.safetensors')
            save_file(initial, str(initial_file)); initial_hash = file_hash(initial_file)
            batches = paired_batches(rows, rep)
            for arm in ARMS:
                key = f'r{rep}-{arm}'; load_adapter(modules, initial)
                optimizer = torch.optim.AdamW(parameters, lr=CONFIG['learningRate'], betas=tuple(CONFIG['betas']),
                    eps=CONFIG['epsilon'], weight_decay=CONFIG['weightDecay'], foreach=False)
                writer.write(dict(event='training_start', key=key, initializationHash=initial_hash,
                                  trainableParameters=sum(v.numel() for v in parameters)))
                ...
                for step, batch in enumerate(batches, 1):
                    ...
                    for pair in batch:
                        ...
                        for side in ('left', 'right'):
                            example = pair[side]; h = study.digest(example)
                            if h not in cache:
                                cache[h] = encode_confidence_training(tokenizer, example, model.device,
                                                                     max_input_tokens=CONFIG['maxTrainingTokens'])
                            items.append(cache[h])
                        encoded.append(items)
                    torch.cuda.synchronize(); tick = time.perf_counter()
                    metrics = train_paired_step(model, optimizer, parameters, encoded, eos, codes, arm)
                    torch.cuda.synchronize(); metrics['seconds'] = time.perf_counter()-tick
                ...
                event = dict(event='training_complete', key=key, steps=144, checkpoint=checkpoint.name,
                    sha256=file_hash(checkpoint), initializationHash=initial_hash, trainingSeconds=time.perf_counter()-started)
                writer.write(event); checkpoints[key] = event
        print('All nine paired-confidence adapters frozen before test.', flush=True)
        ...
        for call in p['calls']:
            ...
            if key != current:
                ...
                current = key
            request = study.request_for(p, call['id'], answers, checkpoints); writer.write(request)
            started = time.perf_counter()
            if call['kind'] == 'answer':
                text, metrics, trace = trace_generation(model, tokenizer, request['messages'], call['seed'], settings=p['generation'])
                event = dict(event='answer', id=call['id'], status='ok', text=text, seconds=time.perf_counter()-started,
                             metrics=metrics, trace=trace, errorType=None)
                validate_answer(event, request); answers[(call['task'], call['producer'])] = event
            else:
                task = p['tasks'][call['task']]; answer = answers[(task['id'], call['producer'])]
                scores = assess_answer(model, tokenizer, task['question'], answer['text'], max_input_tokens=1792)
            ...
            writer.write(event)
            if (call['id']+1) % 192 == 0: print(f'{call["id"]+1}/{p["plannedCalls"]} calls recorded', flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('journal', type=Path); parser.add_argument('data', type=Path)
