...
Position readings glitch or fail; the felt displacement is noisy; energy runs down and is
restored on a charging square; two objects carry a hue whose value is only learned by
tasting; hue can only be read where the attention spotlight lands, and salient events can
capture the spotlight without the agent seeing it move. Protocol: docs/INDICATOR_AGENT_PROTOCOL.md.
...
STAY = 4
N_ACTIONS = 5
GLITCH = 0.2  # among the steps with a reading
BLACKOUT = 0.15
FELT_OK = 0.9
ENERGY_STEP = 1 / 14
ENERGY_NOISE = 0.05
...
def value(theta):
    """Value of a hue: a single peak at 0.1 on the circle, between -1 and 1."""
    return 2.0 * np.exp(3.0 * (np.cos(2 * np.pi * (np.asarray(theta) - 0.1)) - 1.0)) - 1.0
...
class SenseAtelier:
    ...
    def __init__(self, seed, mode="fixed", change_step=24):
        ...
        streams = [np.random.default_rng(s) for s in rng.integers(2 ** 63, size=12)]
        start, body, flash, reading, felt, energy, capture, cue, hue_noise, spawn_square, spawn_hue, change = streams
        ...
        u = reading.random(T)
        self.blackout = u < BLACKOUT
        self.glitch = (u >= BLACKOUT) & (u < BLACKOUT + GLITCH * (1 - BLACKOUT))
    ...
    def _observe(self, felt, onsets, intent, reward):
        t = self.t
        truth = {"t": t, "p": self.p, "d": self.d, "energy": self.energy, "objects": dict(self.objects)}
        if self.blackout[t]:
            read = None
        elif self.glitch[t]:
