"""Cross-task Qwen interchange; frozen checkpoints and both question directions."""
import json
import os
...
def run_group(model,tokenizer,pair,choices):
    ...
    for request in study.requests():
        ...
        if kind=='intact':
            ...
            donor=recipient if kind=='sham' else cache[(request.get('donorTask',task),'donor',request['donorState'],request['donorCode'])]
            with torch.inference_mode():
...
def run(path,parent,interchange,*,resume=False):
    from safetensors.torch import load_file
    from transformers import AutoModelForCausalLM,AutoTokenizer
    from research.cross_model_gpu import environment
    path,parent,interchange=Path(path),Path(parent),Path(interchange)
    if file_hash(parent)!=study.PARENT_SHA256:raise ValueError('Parent journal changed')
    if file_hash(interchange)!=study.INTERCHANGE_SHA256:raise ValueError('Experiment-15 journal changed')
    ...
    for index,group in enumerate(fixed['groups'][len(done):],len(done)+1):
        ...
        print(f'Group {index}/{len(fixed["groups"])}; {index*232}/{fixed["plannedForwards"]} recorded forwards',flush=True)
    if file_hash(parent)!=study.PARENT_SHA256:raise ValueError('Parent journal changed during execution')
    if file_hash(interchange)!=study.INTERCHANGE_SHA256:raise ValueError('Experiment-15 journal changed during execution')
    for key,sha in frozen.items():
...
if __name__=='__main__':
    ...
    p.add_argument('--parent-journal',type=Path,required=True);p.add_argument('--interchange-journal',type=Path,required=True);p.add_argument('--resume',action='store_true')
    a=p.parse_args();run(a.journal,a.parent_journal,a.interchange_journal,resume=a.resume)
