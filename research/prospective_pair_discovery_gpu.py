"""Collect semantic coding, then prospective forecasts, then private-cache tasks."""
import argparse
import hashlib
import json
import os
...
def run(path):
    ...
    try:
        ...
        reference=study.parent_data(tokenizer)
        ...
        for case in p['cases']:
            ...
            mask=study.pair_swap(tokenizer,case,trace['promptTokenIds'],len(actual.prefix))
            ...
            restored,restore_record=permute_prefix_cache(model,altered,permutation=indices,mode='values_only',**args)
            ...
            branches=study.compile_branches(tokenizer,actual.prefix,case,p); cache_export=None
        ...
        for phase in study.PHASES:
            ...
            for call in study.calls(p,phase):
                state=held[call['case']]; name=study.branch_name(phase,call); branch=state['branches'][name]
                snapshot=state['states'][call['condition']]
                ...
                writer.write(event); recorded.append(event); by_key[(call['case'],name,call['condition'])]=decoded
                if phase=='task' and call['condition']=='actual' and call['target']==p['cases'][call['case']]['targetIndex']:
                    require(decoded==reference['tasks'][(call['case'],'actual')]['decoded'],'Parent task reproduction failed')
                original=by_key.get((call['case'],name,'actual'))
                for control in ('same_schedule','restored'):
                    other=by_key.get((call['case'],name,control))
                    if original is not None and other is not None:
                        require(original==other,'Exact branch control failed; preserve attempt')
                if len(recorded)%100==0: print(f'{len(recorded)}/{p["counts"][phase]} {phase} records',flush=True)
            require(parameters==_parameters(model),'Parameters changed')
            if phase!='task':
                event='coding_complete' if phase=='coding' else 'forecasts_complete'
                writer.write(dict(event=event,count=len(recorded),recordsHash=study.digest(recorded)))
                print(f'All {len(recorded)} {phase} records durably frozen before the next phase.',flush=True)
        writer.write(dict(event='complete',**p['counts'])); terminal=True
        ...
        with path.with_suffix('.summary.json').open('x',encoding='utf-8',newline='\n') as f:
            f.write(json.dumps(summary,ensure_ascii=False,indent=2,allow_nan=False)+'\n')
        print(json.dumps(dict(complete=True,**p['counts'])),flush=True)
        return summary
