...
def cache_audit(directory,state):
    ...
    for name in ('values_permuted','joint_permuted','restored'):
        ...
        indices=state['bindingMask']['permutation']; indices=inverse_permutation(indices) if name=='restored' else indices
...
def verify(path,tokenizer):
    ...
    return dict(schema='menia-prospective-binding-discovery-audit-v1',verified=True,journalSHA256=hashlib.sha256(path.read_bytes()).hexdigest(),
        planHash=report['planHash'],sourceHash=report['sourceHash'],chainEnd=report['chainEnd'],
        recomputedSummary=True,allForecastsPrecedeTasks=True,parentStatesExactlyReproduced=24,parentBranchesExactlyReproduced=48,exportedCacheAudits=caches,
