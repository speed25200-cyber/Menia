...
from make_presence_detection_notebook import make_presence_notebook
from tests_bootstrap.test_activation_launcher import FakeRunner, REVISION


class PresenceRunner(FakeRunner):
    def run(self,stage,command,cwd=None):
        result=super().run(stage,command,cwd);command=[str(c) for c in command]
        if 'research.presence_detection_gpu' in command:
            path=Path(command[command.index('research.presence_detection_gpu')+1])
...
class PresenceLauncherTests(unittest.TestCase):
    def test_one_self_contained_cell_with_frozen_revision(self):
        n=make_presence_notebook(REVISION);cells=[c for c in n['cells'] if c['cell_type']=='code']
        self.assertEqual(len(cells),1);source=''.join(cells[0]['source']);compile(source,'presence-colab','exec')
        self.assertIn(REVISION,source);self.assertIn("profile='presence'",source);self.assertIn("storage='local'",source)

    def test_all_nine_adapters_exported_and_resume_uses_same_run_without_drive(self):
        with tempfile.TemporaryDirectory() as tmp,contextlib.redirect_stdout(io.StringIO()):
            runners=[]
            def factory(path):
                r=PresenceRunner(path);runners.append(r);return r
            def no_drive(path): raise AssertionError('Unexpected Drive mount')
            for _ in range(2):
                r=launch_activation(REVISION,'# fixture',content=Path(tmp),profile='presence',storage='local',
                    runner_factory=factory,mount=no_drive,download=lambda p:None)
                self.assertEqual(r['status'],'completed')
            self.assertIn('--resume',next(c for c in runners[-1].calls if 'research.presence_detection_gpu' in c))
            with zipfile.ZipFile(Path(tmp)/'menia-detection-presence.zip') as z:
    ...
    def test_failure_preserves_diagnostics_and_available_checkpoints(self):
        class Partial(PresenceRunner):
            def run(self,stage,command,cwd=None):
                result=super().run(stage,command,cwd)
                if 'research.presence_detection_gpu' in [str(c) for c in command]: raise RuntimeError('Fixture interruption')
                return result
        with tempfile.TemporaryDirectory() as tmp,contextlib.redirect_stdout(io.StringIO()):
            r=launch_activation(REVISION,'# fixture',content=Path(tmp),profile='presence',storage='local',
