...
## Expériences Atelier : un agent cherche-t-il la cause de son propre corps ?

Trois expériences pré-enregistrées, exécutées sur CPU, avec audits indépendants.
[Enquête sur l'origine](docs/ORIGIN_INQUIRY_RESULTS.md) : un agent entraîné
uniquement à prédire ses observations, jamais informé que son corps a une
cause cachée, dirige 99 à 100 % de ses inspections vers la marque qui la
révèle, la décode et agit d'après elle ; zéro enquête quand l'origine est
donnée ou sans trace ; huit critères sur huit. [Enquête apprise](docs/LEARNED_INQUIRY_RESULTS.md) :
la même disposition s'apprend par renforcement, avec dose-effet, sous une
aversion à agir dans l'ignorance ; le bonus d'information seul et la surprise
